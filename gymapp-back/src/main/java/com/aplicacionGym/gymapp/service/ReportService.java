...
public class ReportService {
    ...
    public void generateMonthlyReport(HttpServletResponse response) throws IOException {
        ...
        Paragraph date = new Paragraph(
                "Emitido el: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")) + " hs",
                subTitleFont);
        ...
        addMetricCell(table, "Promedio por Cliente",
                "$" + String.format("%.2f",
                        stats.getMonthlyRevenue() / (stats.getActiveClients() > 0 ? stats.getActiveClients() : 1)),
                metricValueFont);
    ...
    private void addMetricCell(PdfPTable table, String label, String value, Font valueFont) {
        PdfPCell labelCell = new PdfPCell(
                new Phrase(label, FontFactory.getFont(FontFactory.HELVETICA_BOLD, 12, Color.DARK_GRAY)));
