...
- **Este ADR cubre Capa 2 (el profesor pidiéndole cosas al LLM).** Surgió una idea para Capa 3 (22/09/2026): que el alumno genere su propia rutina sin pasar por el profesor. Ese caso NO queda resuelto acá — si se define, necesita su propia decisión (probablemente el mismo principio de revisión humana, pero cambia quién revisa si no hay un profesor en el medio). Ver nota en `PRD_ENTRENADOR.md` § Abierto para la próxima revisión.
